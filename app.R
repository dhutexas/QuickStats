      ...
      mainPanel(
        ...
        tabPanel('About',
                 ...
                 p('The code for this web application is available at',
                   a('GitHub.', href='https://github.com/dhutexas/QuickStats', target="_blank")),
                 p('If you want to run this code on your computer (in a local R session), run the code below:',
                   br(),
                   code('library(shiny)'),br(),
                   code('runGitHub("QuickStats","dhutexas")')
                 ),
                 br(),
                 strong('Author'),
...
server <- function(input, output) {
  ...
  output$anova = renderPrint({
    df = datasetInput()
    df = df %>% select(input$var1, input$var2)
    names = colnames(df)
    x = names[1]
    y = names[2]
    fit = aov(df[,2]~df[,1])
    out = data.frame(val = unclass(summary(fit)))
    rownames(out) = c(x,'Residuals')
    print(paste0("Predicting ", y,'(y) from ',x,' (x)'))
    print(out)
  })
  ...
  output$aci = renderPrint({
    ...
    x = names[1]
    y = names[2]
    fit = confint(aov(df[,2]~df[,1]))
    out = data.frame(val = unclass(fit))
  ...
  output$anova2 = renderPrint({
    ...
    y = names[2]
    x2 = names[3]
    fit = aov(df[,2]~df[,1]+df[,3])
    out = data.frame(val = unclass(summary(fit)))
    rownames(out) = c(x,x2,'Residuals')
    print(paste0("Predicting ", y,'(y) from ',x,' (x1) and ',x2,' (x2)'))
    print(out)
  })
  ...
  output$anova3 = renderPrint({
    ...
    x2 = names[3]
    x3 = names[4]
    fit = aov(df[,2]~df[,1]+df[,3]+df[,4])
    out = data.frame(val = unclass(summary(fit)))
    rownames(out) = c(x,x2,x3,'Residuals')
    print(paste0("Predicting ", y,'(y) from ',x,' (x1), ',x2,'(x2), and ',x3,' (x3)'))
    print(out)
  })
